...
// Rejects addresses whose domain has no valid MX/A record (e.g. throwaway domains used by bots to farm confirmation emails), on top of Assert\Email's format-only check - applied automatically by FormSubmissionType to every TYPE_EMAIL field (contact/register/reset-password-request alike), formerly a per-app scaffold copy (App\Validator\Constraints\DnsEmail)
class DnsEmailValidator extends ConstraintValidator
{
    ...
    public function validate(mixed $value, Constraint $constraint): void
    {
        ...
        // Format check first, and never cached: it's cheap and specific to this exact value, unlike the DNS check below which only depends on the domain part
        ...
        $domainAcceptsMail = $this->cache->get(
            ...
            static function (ItemInterface $item) use ($domain, $emailValidator): bool {
                ...
                // DNSCheckValidation only ever inspects the domain part, so a synthetic local part is fine here and lets the result be cached per domain, not per address
