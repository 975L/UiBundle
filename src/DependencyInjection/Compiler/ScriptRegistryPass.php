...
class ScriptRegistryPass implements CompilerPassInterface
{
    public function process(ContainerBuilder $container): void
    {
        if (!$container->has(ScriptRegistry::class)) {
            return;
        }

        $registry = $container->getDefinition(ScriptRegistry::class);

        $tagged = $container->findTaggedServiceIds('ui.script');
