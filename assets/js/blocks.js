...
(function () {
    ...
    window.addEventListener('DOMContentLoaded', uiInitAll);
...
// ===== Kind selector: load the data sub-form via AJAX =====

document.addEventListener('change', function (event) {
    const select = event.target;
    if (!select.dataset.blockKindUrlValue) return;

    const kind     = select.value;
    const prefix   = select.name.replace(/\[kind\]$/, '');
    const kindRow  = select.closest('[data-kind-row]');
    ...
    fetch(select.dataset.blockKindUrlValue + '?k=' + encodeURIComponent(kind))
        .then(r => r.text())
        .then(html => {
