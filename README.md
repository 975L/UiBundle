...
`/management/ui/block/gallery` (EasyAdmin, `ROLE_EDITOR`, linked from the sidebar's "Links" section - see `c975L\UiBundle\Management\MenuProvider::getLinks()`) renders every pickable block kind with sample data, grouped by category like the real kind picker. It's meant to help an editor see what a kind actually looks like before choosing one, without first creating and filling in a real block on a page.
