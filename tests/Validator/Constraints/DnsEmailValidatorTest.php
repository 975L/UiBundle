...
use c975L\UiBundle\Validator\Constraints\DnsEmailValidator;
...
class DnsEmailValidatorTest extends ConstraintValidatorTestCase
{
    ...
    // gmail.com has a stable MX record, so this exercises the "domain accepts mail" path without depending on a domain we control
    ...
    // ".invalid" is reserved by RFC 2606 to never resolve, so this is a deterministic, non-flaky way to exercise the rejection path
    ...
    // The DNS check result is cached per domain (not per address), so a repeated hit on the same domain (bot flood, or an admin re-saving the same user) skips the live lookup
