...
class CollectionExtensionTest extends TestCase
{
    private function createExtension(
        ...
        return new CollectionExtension($sourceRegistry, $blockExtension, $requestStack, $urlGenerator);
    }

    // Each CollectionItem becomes a never-persisted "collection_item" Block, rendered through the
    // exact same render_block() pipeline as a real, editor-placed block
    public function testRenderItemsBuildsACollectionItemBlockPerItemAndRendersItThroughBlockExtension(): void
    {
        ...
        $extension = $this->createExtension($sourceRegistry, $blockExtension);
        $result = $extension->renderItems('site.collection.projects', 6, null);
    ...
    // detailPage configured, item has a slug, and a "page" route parameter is available: the item's
    // detail link is built from the current page's own slug, never the detail Page's own slug
    public function testRenderItemsBuildsDetailUrlWhenDetailPageAndItemSlugAreBothSet(): void
    {
        ...
        $extension = $this->createExtension($sourceRegistry, $blockExtension, $request);
        $extension->renderItems('site.collection.projects', null, 'project-detail');

        $this->assertSame('/pages/projects/project-a', $renderedBlock->getData()['detailUrl']);
    }

    // The parent page itself is being previewed (page_preview route): the detail link must follow
    // it onto /preview too, so an editor can reach an unpublished detail before going live
    ...
    public function testRenderItemsLeavesDetailUrlNullWhenDetailPageIsNotSet(): void
    {
        ...
        $extension = $this->createExtension($sourceRegistry, $blockExtension, $request);
        $extension->renderItems('site.collection.projects', null, null);

        $this->assertNull($renderedBlock->getData()['detailUrl']);
    }

    // detailPage configured but the item's own source never supplies a slug: no link either, same
    // tolerant degradation as a source with no "detail" capability at all
    ...
    public function testRenderItemsReturnsEmptyArrayWhenSourceHasNoItems(): void
    {
        ...
        $extension = $this->createExtension($sourceRegistry, $this->createStub(BlockExtension::class));

        $this->assertSame([], $extension->renderItems('site.collection.projects', null, null));
    }
