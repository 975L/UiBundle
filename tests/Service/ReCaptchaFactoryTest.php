...
// Moved from ContactFormBundle alongside the class it tests - see UPGRADE.md
class ReCaptchaFactoryTest extends TestCase
{
    ...
    // Regression guard: 0 is a deliberate, valid score threshold (accept everything), not "unset" - must not
    // be silently overridden by the fallback the same way an empty string is
    public function testCreateRespectsAnExplicitZeroScoreThreshold(): void
    {
        $factory = $this->createFactory([
            'recaptcha3-score-threshold' => '0',
        ]);

        $reCaptcha = $factory->create('fallback-secret', new Post(), 0.5);

        $this->assertSame(0.0, $this->readProperty($reCaptcha, 'threshold'));
    }
