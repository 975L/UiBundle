...
// Only covers the deterministic, IO-free parts of the Maker - generate()/interact() delegate to
// MakerBundle's own Generator/ConsoleStyle (file writing, prompting) which need a real booted app
// to exercise meaningfully, so they're verified manually via "bin/console c975l:ui:block:create"
class MakeBlockCommandTest extends TestCase
{
    ...
    // REQUIRED, not OPTIONAL: interact() fills it in before Console's own argument validation runs
    // for an interactive session, but --no-interaction skips interact() - REQUIRED lets Console itself
    // reject a missing argument with a clear message instead of generate() crashing on a null value
